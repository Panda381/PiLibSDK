...
// === save output file
	...
	else
	{
		...
		(&fmt->wBitsPerSample)[2] = 0;
	}

	// sound format
	fprintf(f, "// sound format: %s\n", 
		(fmt->wBitsPerSample == 4) ?
			((fmt->nChannels == 1) ? "SNDFORM_ADPCM" : "SNDFORM_ADPCMS") :
			((fmt->wBitsPerSample == 8) ?
				((fmt->nChannels == 1) ? "SNDFORM_PCM8" : "SNDFORM_PCM8S") :
				((fmt->nChannels == 1) ? "SNDFORM_PCM16" : "SNDFORM_PCM16S")));

	// relative sound speed
	fprintf(f, "// sound speed relative to 50000Hz: %.5ff\n", (double)fmt->nSamplesPerSec/50000);

/*
	Sound header (size 12 bytes):
typedef struct {
...
*/
	...
	// data header
	fprintf(f, "const u8 %s[%d] __attribute__ ((aligned(4))) = {", argv[3], n+12);

	// header
	fprintf(f, "\n\t0x%02X, 0x%02X, 0x%02X, 0x%02X,\t// data size=%u", (u8)n, (u8)(n>>8), (u8)(n>>16), (u8)(n>>24), n);
	fprintf(f, "\n\t12, 0,\t\t\t// data offset");
	i = fmt->nSamplesPerSec;
	fprintf(f, "\n\t0x%02X, 0x%02X,\t\t// sample rate=%u", (u8)i, (u8)(i>>8), i);
	i = (&fmt->wBitsPerSample)[2];
	fprintf(f, "\n\t0x%02X, 0x%02X,\t\t// ADPCM samples per block=%u", (u8)i, (u8)(i>>8), i);
	fprintf(f, "\n\t%d,\t\t\t// bits per sample", fmt->wBitsPerSample);
	fprintf(f, "\n\t%d,\t\t\t// number of channels", fmt->nChannels);
