...
#define SECT_SIZE 512  		// 512 bytes
...
typedef struct {
	...
	sDir*	dir;		// pointer to directory entry in disk buffer DISKBUF_DIR
...
// index of disk buffer
enum {
	DISKBUF_BOOT = 0,	// FSInfo sector, boot sector
	DISKBUF_FAT,		// FAT sector
	DISKBUF_DIR,		// directory sector
	DISKBUF_DATA,		// file data sector

	DISKBUF_NUM		// number of buffers
};

// disk buffers
extern ALIGNED u8 DiskBuf[SECT_SIZE*DISKBUF_NUM];

// current sector in disk buffer (SECT_NONE = none)
#define SECT_NONE (~0u) // none sector
extern u32 DiskBufSect[DISKBUF_NUM];

// dirty flag - sector in disk buffer is modified and should be saved
extern Bool DiskBufDirty[DISKBUF_NUM];
...
// default FAT date and time 1/1/2026 12:00
#define FATDATE() (1 + (1 << 5) + ((2026 - 1980) << 9))
