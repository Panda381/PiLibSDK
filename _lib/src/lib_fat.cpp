...
// disk buffers
ALIGNED u8 DiskBuf[SECT_SIZE*DISKBUF_NUM];

// current sector in disk buffer (SECT_NONE = none)
u32 DiskBufSect[DISKBUF_NUM] = { SECT_NONE, SECT_NONE, SECT_NONE, SECT_NONE };

// dirty flag - sector in disk buffer is modified
Bool DiskBufDirty[DISKBUF_NUM] = { False, False, False, False };
...
// - using 'bufinx' buffer
Bool Disk_FlushBuf(int bufinx)
{
	if (DiskBufDirty[bufinx])
	{
		// reset dirty flag (in case of error too, maybe write-protected)
		DiskBufDirty[bufinx] = False;

		// write sector to disk
		if (!SD_WriteSect(DiskBufSect[bufinx], &DiskBuf[SECT_SIZE*bufinx])) return False;

		// write 2nd copy of FAT
		if (	(DiskFS != FS_NONE) &&	// valid file system?
			((u32)(DiskBufSect[bufinx] - FatBase) < FatSizeSect) && // is it 1st copy of FAT?
			(FatNum == 2)) // 2 FATs ?
			SD_WriteSect(DiskBufSect[bufinx] + FatSizeSect, &DiskBuf[SECT_SIZE*bufinx]); // write into 2nd FAT
	}
	return True;
}
...
Bool Disk_SyncFS()
{
	// flush disk buffer
	int bufinx;
	for (bufinx = 0; bufinx < DISKBUF_NUM; bufinx++)
	{
		// - using 'bufinx' buffer
		if (!Disk_FlushBuf(bufinx)) return False;
	}

	// synchronize FSinfo (not if disabled)
	if ((DiskFS == FS_FAT32) && (FSinfo == FSI_DIRTY))
	{
		memset(&DiskBuf[SECT_SIZE*DISKBUF_BOOT], 0, SECT_SIZE); // clear disk buffer
		sFSinfo* fs = (sFSinfo*)&DiskBuf[SECT_SIZE*DISKBUF_BOOT];
		...
		DiskBufSect[DISKBUF_BOOT] = DiskBase + 1; // 2nd sector immediately after boot sector
		SD_WriteSect(DiskBufSect[DISKBUF_BOOT], &DiskBuf[SECT_SIZE*DISKBUF_BOOT]);
...
// - using 'bufinx' buffer
Bool Disk_MoveBuf(u32 sect, int bufinx)
{
	if (sect != DiskBufSect[bufinx])
	{
		...
		// read new sector
		if (!SD_ReadSect(sect, &DiskBuf[SECT_SIZE*bufinx]))
		{
			DiskBufSect[bufinx] = SECT_NONE;
			return False;
		}

		// new sector is valid
		DiskBufSect[bufinx] = sect;
	}
	return True;
}
...
u32 Disk_ReadFat(u32 clust)
{
	...
	case FS_FAT12:	// FAT12
		...
		// read sector
		// - using 'bufinx' buffer
		if (!Disk_MoveBuf(sect, DISKBUF_FAT)) return SECT_NONE;

		// 1st part of the entry
		n = DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_FAT];
		...
		n |= (u16)DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_FAT] << 8;
	...
	case FS_FAT16:	// FAT16
		...
		// read entry
		return *(u16*)&DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_FAT];
	...
	case FS_FAT32:	// FAT32
		...
		// read entry
		return *(u32*)&DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_FAT] & 0x0fffffff;
...
Bool Disk_WriteFat(u32 clust, u32 val)
{
	...
	case FS_FAT12:	// FAT12
		...
		// read sector
		// - using 'bufinx' buffer
		if (!Disk_MoveBuf(sect, DISKBUF_FAT)) return False;

		// set 1st part of the entry
		p = &DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_FAT];
		*p = (u8)(((clust & 1) != 0) ? ((*p & 0xf) | (val << 4)) : val);

		// set dirty flag
		DiskBufDirty[DISKBUF_FAT] = True;
	...
	case FS_FAT16:	// FAT16
		...
		// set entry
		*(u16*)&DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_FAT] = (u16)val;
	...
	case FS_FAT32:	// FAT32
		...
		// set entry
		p32 = (u32*)&DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_FAT];
...
Bool Disk_ClearFat(u32 clust, u32 prev)
{
	...
	{
		// - using DISKBUF_FAT buffer
		if (!Disk_WriteFat(prev, ~0U)) return False;
        }
...
u32 Disk_CreateFat(u32 clust)
{
	...
	// - using DISKBUF_FAT buffer
	if (!Disk_WriteFat(nc, ~0U)) return SECT_NONE;
...
// - using DISKBUF_DIR buffer
Bool Disk_DirClear(u32 clust)
{
	// flush disk buffer
	// - using 'bufinx' buffer
	if (!Disk_FlushBuf(DISKBUF_DIR)) return False;
	...
	// prepare sector
	DiskBufSect[DISKBUF_DIR] = sect;
	memset(&DiskBuf[SECT_SIZE*DISKBUF_DIR], 0, SECT_SIZE);

	// write sectors of the cluster
	u8 num = ClustSizeSect;
	for (; num > 0; num--)
	{
		if (!SD_WriteSect(sect, &DiskBuf[SECT_SIZE*DISKBUF_DIR])) return False;
		sect++;
	}
...
// - using DISKBUF_FAT buffer, prepare 'dir' pointer to DISKBUF_DIR buffer
Bool Disk_DirInx(sFile* dir, u32 off)
{
	...
	dir->dir = (sDir*)&DiskBuf[(off & SECT_MASK) + SECT_SIZE*DISKBUF_DIR];
...
// - using DISKBUF_FAT and DISKBUF_DIR buffers, shifts 'dir' pointer in DISKBUF_DIR buffer
...
// - sets: off, clust, sect, dir
// - disk buffer DISKBUF_DIR contains valid directory entry (pointed by dir->dir)
// - using DISKBUF_FAT and DISKBUF_DIR buffers, shifts 'dir' pointer in DISKBUF_DIR buffer to valid free entry
Bool Disk_DirAlloc(sFile* dir)
{
	...
	for (;;)
	{
		...
		// - using 'bufinx' buffer
		if (!Disk_MoveBuf(dir->sect, DISKBUF_DIR)) return False;
...
// - using DISKBUF_FAT and DISKBUF_DIR buffers, shifts 'dir' pointer in DISKBUF_DIR buffer to valid entry
...
// find directory entry by the name, volume excluded, buffer DISKBUF_DIR (returns False on error or if not found)
// - requires valid sclust (start cluster of the directory, 0 = root)
...
// create directory entry, buffer DISKBUF_DIR (returns False on error)
// - start cluster dir->sclust must be valid (or 0 = root)
// - requires dir->name to contain name of new entry
// - sets: off, clust, sect, dir
// - disk buffer contains created directory entry (pointed by dir->dir)
// - using DISKBUF_FAT and DISKBUF_DIR buffers, creates valid entry in DISKBUF_DIR buffer
...
// - using DISKBUF_DIR buffers, creates valid entry in DISKBUF_DIR buffer
...
// get file info from directory entry, buffer DISKBUF_DIR (returns False on error)
// - requires valid dir->sect and valid directory entry in DISKBUF_DIR sector buffer (dir->dir)
...
// - using DISKBUF_FAT and DISKBUF_DIR buffers
...
// - using DISKBUF_BOOT buffer
u8 Disk_CheckFS(u32 sect)
{
	// load boot sector
	// - using 'bufinx' buffer
	if (!Disk_MoveBuf(sect, DISKBUF_BOOT)) return FS_NUM;
	sBoot16* b = (sBoot16*)&DiskBuf[SECT_SIZE*DISKBUF_BOOT];
	sBoot32* b2 = (sBoot32*)b;
	...
	if (totsect < syssect) return FS_NONE;

	// number of clusters
	u32 clustnum = (totsect - syssect) / clustsize;
	if (clustnum == 0) return FS_NONE;
...
{
	int i;
	for (i = 0; i < DISKBUF_NUM; i++)
	{
		DiskBufSect[i] = SECT_NONE; // no sector in disk buffer
		DiskBufDirty[i] = False; // no dirty data in disk buffer
	}
...
Bool DiskMount()
{
	...
	{
		// - using 'bufinx' buffer
		if (Disk_MoveBuf(sect+1, DISKBUF_BOOT))
		{
			FSinfo = 0;
			sFSinfo* fi = (sFSinfo*)&DiskBuf[SECT_SIZE*DISKBUF_BOOT];
...
FILECREATEERR:
	...
	file->dirsect = DiskBufSect[DISKBUF_DIR];
	...
	// write disk buffer
	// - using 'bufinx' buffer
	return Disk_FlushBuf(DISKBUF_DIR);
}
...
// - using DISKBUF_FAT and DISKBUF_DATA buffers
u32 FileRead(sFile* file, void* buf, u32 num)
{
	...
	for (; read < num; )
	{
		...
		else
		{
			...
			// - using 'bufinx' buffer
			if (!Disk_MoveBuf(file->sect, DISKBUF_DATA)) break;
			memcpy(buf, &DiskBuf[(file->off & SECT_MASK) + SECT_SIZE*DISKBUF_DATA], n);
		}
...
u32 FileWrite(sFile* file, const void* buf, u32 num)
{
	...
	for (; write < num; )
	{
		...
		else
		{
			...
			// - using 'bufinx' buffer
			if (!Disk_MoveBuf(file->sect, DISKBUF_DATA)) break;
			memcpy(&DiskBuf[(file->off & SECT_MASK) + SECT_SIZE*DISKBUF_DATA], buf, n);
			DiskBufDirty[DISKBUF_DATA] = True;
		}
...
// callback - relative seek file
void StreamSeekFile(sStream* str, int off)
{
	sFile* f = (sFile*)str->cookie;
	off = f->off + off;
	if (off < 0) off = 0;
...
u32 FilePrintArg(sFile* file, const char* fmt, va_list args)
{
	...
	wstr.seek = StreamSeekFile; // seek
	wstr.cookie = (size_t)file;
...
// callback - read data from file
u32 StreamReadFile(sStream* str, void* buf, u32 num)
{
	sFile* f = (sFile*)str->cookie;
	num = FileRead(f, buf, num);
	str->pos = f->off;
	return num;
}
...
void StreamReadFileInit(sStream* str, sFile* file)
{
	...
	str->size = file->size;
	str->pos = file->off;
	str->cookie = (size_t)file;
	str->read = StreamReadFile;
	str->seek = StreamSeekFile; // seek
}
...
void StreamWriteFileInit(sStream* str, sFile* file)
{
	...
	str->pos = file->off;
	str->cookie = (size_t)file;
	str->write = StreamWriteFile;
	str->seek = StreamSeekFile; // seek
}
#endif // USE_STREAM

// flush file writes and flush disk buffers (returns False on error)
// - using DISKBUF_DIR buffer, flush all buffers
Bool FileFlush(sFile* file)
{
	...
	if ((file->attr & ATTR_MODI) != 0)
	{
		...
		// - using 'bufinx' buffer
		if (!Disk_MoveBuf(file->dirsect, DISKBUF_DIR)) return False;
		sDir* d = file->dir;
...
u16 GetDir(char* buf, u16 len)
{
	...
	while ((DirTmp.sclust != 0) && ((DiskFS != FS_FAT32) || (DirTmp.sclust != DirBase)))
	{
		...
		// - using 'bufinx' buffer
		if (!Disk_MoveBuf(DirTmp.sect, DISKBUF_DIR)) return 0;
...
// - using DISKBUF_FAT and DISKBUF_DIR buffers, prepare 'dir' pointer to DISKBUF_DIR buffer
...
Bool SetFileSize(sFile* file, u32 size)
{
	...
	// write disk buffer
	// - using 'bufinx' buffer
	return Disk_FlushBuf(DISKBUF_FAT);
}
...
Bool FileDelete(const char* path)
{
	...
	// write disk buffer
	// - using 'bufinx' buffer
	return Disk_FlushBuf(DISKBUF_DIR) && Disk_FlushBuf(DISKBUF_FAT);
}
...
Bool DirCreate(const char* path)
{
	...
	sDir* dir = (sDir*)&DiskBuf[SECT_SIZE*DISKBUF_DIR];
...
Bool FileMove(const char* oldpath, const char* newpath)
{
	...
	if (((attr & ATTR_DIR) != 0) && (DirTmp.sclust != pclust))
	{
		...
		// load start sector
		// - using 'bufinx' buffer
		if (!Disk_MoveBuf(sect2, DISKBUF_DIR)) return False;

		// second entry must be '..'
		dir2 = (sDir*)&DiskBuf[(1 * DIR_SIZE) + SECT_SIZE*DISKBUF_DIR];
		if (dir2->name[0] != '.') return False;

		// set new parent cluster
