...
#if USE_DRAW_STDIO	// use DRAW stdio (printf() or DrawPrint() function)
	...
	for (;;)
	{
		...
		while (!stop)
		{
			...
			case 't': // technical
				...
				// infinity INF
				if (CheckInfD(dbl) || CheckSInfD(dbl))
				{
					if (ch <= 'Z') // 'A', 'E', 'F', 'G'
						chp = (char*)"INF"; // upper case
					else
						chp = (char*)"inf"; // lower case
				...
				// invalid NAN
				if (CheckNanD(dbl) || CheckSNanD(dbl))
				{
					if (ch <= 'Z') // 'A', 'E', 'F', 'G'
						chp = (char*)"NAN"; // upper case
					else
						chp = (char*)"nan"; // lower case
					size = 3; // length = 3 characters
					flags &= ~PRF_ZEROPAD; // do not add zeroes
					stop = True; // stop parsing
					break;
...
//					(((ch == 'F') || (ch == 'f')) && ((expI < -prec))) || // Ff exponent if exp < -precision ... not compatible with other libc
			...
			case 'p':
				nu64 = (size_t)va_arg(args, void*); // get argument
				if (nu64 == 0)
				{
					chp = NULL;
...
null_ptr:
				if (chp == NULL) chp = (char*)"(null)"; // string is NULL
...
extern "C" int WRAPPER_FUNC(vsprintf)(char* buf, const char* fmt, va_list args)
{
	return (int)MemPrintArg(buf, SPRINTF_MAX, fmt, args);
}

extern "C" int WRAPPER_FUNC(vsnprintf)(char* buf, size_t max, const char* fmt, va_list args)
{
...
extern "C" int WRAPPER_FUNC(sprintf)(char* buf, const char* fmt, ...)
{
	int n;
	va_list args;
	va_start(args, fmt);
...
extern "C" int WRAPPER_FUNC(snprintf)(char* buf, size_t max, const char* fmt, ...)
{
	int n;
	va_list args;
	va_start(args, fmt);
...
#if USE_UART1_STDIO	// 1=use UART1 stdio (printf() or UART1_Print() function; 8 bits, 1 stop bit, no parity)
	if ((StdioMask & STDIO_MASK_UART1) != 0)
	{
		UART1_SendChar(ch);
	}
#endif
...
extern "C" int WRAPPER_FUNC(putchar)(int ch)
{
	PrintChar(ch);
	return ch;
}
...
extern "C" int WRAPPER_FUNC(puts)(const char* txt)
{
	return printf("%s\n", txt);
}
...
// formatted print (returns number of characters)
extern "C" int WRAPPER_FUNC(vprintf)(const char* fmt, va_list args)
{
...
// formatted print (returns number of characters)
extern "C" int WRAPPER_FUNC(printf)(const char* fmt, ...)
{
	u32 n;
	va_list args;
	va_start(args, fmt);
...
extern "C" char WRAPPER_FUNC(getchar)()
{
	return GetCharWait();
}
