...
// - When compiling with GCC v15.2, the compiler made an error in the
//   calculation "b = (c*4 + 3)/146097" - it is necessary to either
//   disable optimization for this function with NOOPTIM or disable
//   these optimizations in the compiler settings:
//   -fno-tree-vrp -fno-tree-dominator-opts.
...
void DebTestCalendar64()
{
	...
	j = -1000000;
	DebTestCalendar64Init(MINYEAR2);

	printf("Check long DateTime64:\n");

	while (True)
	{
		i = DebTestCalendar64Step();
		...
		if (j != DebTestDateTime2.year)
		{
			...
			if ((j % 100) == 0) printf("%d\n", j);
/*
			printf("DateTime64: year=%d days=%d", j, GetDaysInYear64(j));
			if (DateTime64IsLeap(&DebTestDateTime2)) printf(" leap"); else printf(" non-leap");;
			if (DateTime64IsGreg(&DebTestDateTime2)) printf(" Greg"); else printf(" Juli");
			if (DateTime64IsW53(&DebTestDateTime2)) printf(" 53_weeks"); else printf(" 52_weeks");
			printf("\n");
*/
		}
		...
		if (i != 0)
		{
			...
			printf("... ERROR %d: year=%d, month=%d, day=%d, dayofweek=%d\n",
...
s16 DateTime64FastCheck(int loops)
{
	...
	for (; loops > 0; loops--)
	{
		...
		dt.nsec = RandS32Max(999999999) / 100 * 100;
		dt.sec = RandS8Max(59);
		dt.min = RandS8Max(59);
		dt.hour = RandS8Max(23);
		dt.day = RandS8MinMax(1, 31);
		dt.mon = RandS8MinMax(1, 12);
		dt.year = RandS16MinMax(MINYEAR2, MAXYEAR2);
