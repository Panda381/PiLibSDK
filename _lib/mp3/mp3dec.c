...
HMP3Decoder MP3InitDecoder(void* mp3stat)
{
	...
	mp3DecInfo = AllocateBuffers(mp3stat);
 ...
 *              zero out the contents of sMP3DecInfo struct
 *
...
{
	sMP3DecInfo *mp3DecInfo = (sMP3DecInfo *)hMP3Decoder;
 ...
 **************************************************************************************/
static int MP3FindFreeSync(const u8 *buf, const u8 firstFH[4], int nBytes)
{
	int offset = 0;
	const u8 *bufPtr = buf;
 ...
 *
 * Inputs:      valid MP3 decoder instance pointer (HMP3Decoder)
 *              pointer to sMP3FrameInfo struct
 *
 * Outputs:     filled-in sMP3FrameInfo struct
 *
 * Return:      none
 *
 ...
 * Return:      none
 **************************************************************************************/
static void MP3ClearBadFrame(sMP3DecInfo *mp3DecInfo, short *outbuf)
{
	int i;
...
// Returns: size without CRC = 4, size with CRC = 6, error = -1
	...
	mp3DecInfo->frameheadersize = 0; // not unpacked
	if (fhBytes < 0)	
		return ERR_MP3_INVALID_FRAMEHEADER;		/* don't clear outbuf since we don't know size (failed to parse header) */
	*inbuf += fhBytes; // size of frame header = 4 or 6
	
	/* unpack side info */
	siBytes = UnpackSideInfo(mp3DecInfo, *inbuf);

// Returns: MPEG1 mono = 17, MPEG1 stereo = 32 bytes, MPEG2 mono = 9, MPEG2 stereo = 17, error = -1

	if (siBytes < 0) {
		MP3ClearBadFrame(mp3DecInfo, outbuf);
		return ERR_MP3_INVALID_SIDEINFO;
	}
	*inbuf += siBytes; // size of side info 9, 17 or 32 bytes
