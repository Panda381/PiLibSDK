...
{
	if (RealUnit == REAL_UNIT_RAD) return;
	if (RealUnit == REAL_UNIT_DEG)
...
{
	if (RealUnit == REAL_UNIT_RAD) return exp;
	if (RealUnit == REAL_UNIT_DEG)
...
int REALNAME(Test9_old)(REAL* num)
{
	REAL tmp;
	int i;
	u8 oldunit = RealUnit;				// save old angle unit
	RealUnit = REAL_UNIT_DEG;				// use DEG unit
	...
	REALNAME(FromRad)(num);			// convert from radians
	RealUnit = oldunit;					// restore angle unit
...
int REALNAME(Test9)(REAL* num)
{
	...
	REALNAME(FromRad)(num);			// convert from radians
	RealUnit = oldunit;					// restore angle unit
	i = 1 + MANT_BITS - REALNAME(CheckDif)(num, &tmp); // check difference
	REALNAME(Sub)(num, num, &tmp);	// difference
	return i;
}
...
int REALNAME(Test9Ext)(REAL* num)
{
	...
	exp = REALNAME(FromRadExt)(num, exp);	// convert from radians
	RealUnit = oldunit;					// restore angle unit
	i = REAL_BITS - REALNAME(CheckDifExt)(num, exp, &tmp, tmp_exp); // check difference
