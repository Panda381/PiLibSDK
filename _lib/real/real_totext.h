...
// ---------------------------------------------------------------------------
// Global flags:
//  RealBase ... current numeric radix base REAL_BASE_*
...
// === Prepare exponent
	...
	if (RealBase == REAL_BASE_DEC) // decimal base
		REALNAME(Mul)(&m, &m, &REALNAME(ConstLog2)); // multiply by log10(2)
	else if (RealBase == REAL_BASE_HEX) // hexadecimal base
		REALNAME(Pow2)(&m, -2);	// divide by 4 (=divide by log2(16) = 4)
	else if (RealBase == REAL_BASE_OCT) // octal radix base
...
// Global flags:
//  RealBase ... current numeric radix base REAL_BASE_*
//  RealExpMode ... current exponent mode EXP_>
//  RealFix ... current fixed decimal places (0..digits, or REAL_FIX_OFF=off)
//  RealCharPlus ... character of positive number ('+', ' ' or 0=do not use)
//  RealCharExp ... character of exponent ('e', 'E' or 0=do not use)
//  RealExpPlus ... character of positive exponent ('+', ' ' or 0=do not use)
//  RealCharDec ... character used as decimal separator (',', '.' or B7=add flag to previous digit)
...
// --- here we have positive mantissa (m) in range 1..9.9999 (or 0) and signed exponent (exp)
	...
	if (RealExpMode == REAL_EXP_ENG) // engineer mode?
	{
		REALNAME(SetSInt)(&tmp, exp);	// load exponent into tmp
		if ((RealBase == REAL_BASE_DEC) || (RealBase == REAL_BASE_OCT)) // dec and oct radix have modulus of exponent = 3
...
Restart:
	...
	if ((RealCharPlus != 0) || neg) totnum--; // reserve 1 position for sign
	if (RealCharDec != B7) totnum--; // reserve 1 position for decimal point
	...
	if (RealMaxDig > 0)
	{
		if (totnum > RealMaxDig) totnum = RealMaxDig; // limit to valid number of digits
	}
	else
	{
		...
		{
		case REAL_BASE_BIN:
			if (totnum > MANT_DIG_BIN + RealMaxDig) totnum = MANT_DIG_BIN + RealMaxDig;
			break;

		case REAL_BASE_OCT:
			if (totnum > MANT_DIG_OCT + RealMaxDig) totnum = MANT_DIG_OCT + RealMaxDig;
			break;

		case REAL_BASE_HEX:
			if (totnum > MANT_DIG_HEX + RealMaxDig) totnum = MANT_DIG_HEX + RealMaxDig;
			break;

		default: // case REAL_BASE_DEC:
			if (totnum > MANT_DIG_DEC + RealMaxDig) totnum = MANT_DIG_DEC + RealMaxDig;
			break;
		}
	}
	...
	exp3;
	if ((RealExpMode == REAL_EXP_EE) || (RealExpMode == REAL_EXP_ENG) || // user requires exponent mode explicitly
		(exp >= totnum2) ||			// number is too big to fit to display (we may add some zeroes after valid digits)
		((RealExpMode == REAL_EXP_AUTO) && (exp >= totnum)) || // at auto mode, no ending zeroes to integer part
		((exp < -3) && (RealExpMode == REAL_EXP_AUTO))) // number is too low and auto mode is on
	{
		...
		if ((totnum2 > 0) && ((exp < 0) || (RealExpPlus != 0)))
		{
			if (dstexp != NULL)
			{
				dstexp--; // shift pointer
		...
		if ((totnum2 > 0) && (RealCharExp != 0) &&
			((RealBase != REAL_BASE_HEX) || ((RealCharExp != 'e') && (RealCharExp != 'E'))))
		{
			if (dstexp != NULL)
			{
				dstexp--; // shift pointer
		...
		if (RealFix != REAL_FIX_OFF)
		{
			// limit decimal digits
			if (RealFix < decnum) decnum = RealFix;
		}
...
// --- prepare normal mode
	...
	else
	{
		...
		if (RealFix != REAL_FIX_OFF)
		{
			if (RealFix < zeronum) zeronum = RealFix; // limit number of zero digits
			if (RealFix < zeronum + decnum) decnum = RealFix - zeronum; // limit number of decimal digits
		}
	}
	...
	if ((totnum > 0) && !restart && !REALNAME(IsZero)(&m))
	{
		...
		if (RealBase == REAL_BASE_DEC)
		{
			...
			if (REALNAME(Comp)(&m, &REALNAME(Const10)) >= 0)
			{
				...
				if (RealExpMode == REAL_EXP_ENG) // engineering mode correction
				{
		...
		// HEX base
		else if (RealBase == REAL_BASE_HEX)
		{
			// add rounding correction to the mantissa
			REALNAME(Set1)(&tmp);
		...
		// OCT base
		else if (RealBase == REAL_BASE_OCT)
		{
			// add rounding correction to the mantissa
			REALNAME(Set1)(&tmp);
		...
		// BIN base
		else // if (RealBase == REAL_BASE_BIN)
		{
			// add rounding correction to the mantissa
			REALNAME(Set1)(&tmp);
...
// enddp ... ending with decimal point
	...
	if ((intnum == 0) && (size2 > 0))
	{
		...
		else
		{
			mantlen++;
			size2--;
			if (dstmant != NULL) *dstmant++ = RealCharDec; // start decimal point
		}
	...
	for (; totnum2 > 0; totnum2--)
	{
		...
		if (RealBase == REAL_BASE_DEC) // DEC base
			REALNAME(MulUInt)(&m, &m, 10); // multiply * 10
		else if (RealBase == REAL_BASE_HEX) // HEX base
			REALNAME(Pow2)(&m, 4); // multiply * 16
		else if (RealBase == REAL_BASE_OCT) // OCT base
			REALNAME(Pow2)(&m, 3); // multiply * 8
		else // if (RealBase == REAL_BASE_BIN) // BIN base
		...
		if ((intnum == 0) && (size2 > 0))
		{
			...
			else
			{
				mantlen++;
				size2--;
				if (dstmant != NULL) *dstmant++ = RealCharDec;
			}
			enddp = True; // ending with decimal point
			endzero = 0;
		}
