...
//      If generator locks during generation constants, recompile with switch ANTILOCK (and then return back!)
...
//#define ANTILOCK			// uncomment this if generator locks on first generation of higher precision
...
#define CACHE_CHEB_SQRT	1	// 1=get Chebyshev coefficients Sqrt() from cache

#include "../real_def.h"			// define

//#ifndef FACT_COEFF
//#define FACT_COEFF (MAXFACT_COEFF/8)
//#endif

//#ifndef CORD_ATAN
//#define CORD_ATAN CORD_ATAN_FNC(MAXCORD_ATAN)
//#endif
...
REAL TempConstMPi05;		// -pi/2
REAL TempConstPi;			// pi
REAL TempConstMPi;			// -pi
REAL TempConstPi2;			// pi*2
REAL TempConstMPi2;			// -pi*2
...
// Cordic
REAL TempConstAtanInit;	// init value of atan table (=1/expansion factor)
REAL TempConstAtan[CORD_ATAN]; // atan coefficients for Cordic
...
void GenTempConst()
{
	...
	EXP *s_exp, temp_exp, temp2_exp, *mm_exp, *r_exp, a_exp, b_exp, c_exp;
	int k, kk, n;
#endif
	...
	// ln(10)
#ifdef ANTILOCK
	REALNAME(FromDouble)(&TempConstLn10, 2.30258509299405);
#else
	...
	// -pi
	REALNAME(Copy)(&TempConstMPi, &TempConstPi);
	REALNAME(Neg)(&TempConstMPi);
	fprintf(stderr, "-pi ");
	...
	// -pi/2
	REALNAME(Copy)(&TempConstMPi05, &TempConstPi05);
	REALNAME(Neg)(&TempConstMPi05);
	fprintf(stderr, "-pi/2 ");
	...
	// -pi/2
	REALNAME(Copy)(&TempConstMPi2, &TempConstPi2);
	REALNAME(Neg)(&TempConstMPi2);
	fprintf(stderr, "-pi*2 ");
	...
	// ln(pi*2)/2
#ifdef ANTILOCK
	REALNAME(FromDouble)(&TempConstLn10, 0.918938533204673);
#else
	...
	for (i = 0; i < FACT_COEFF; i++)
		REALNAME(Pack)(&TempConstFact[i], (const REAL*)&REALNAME(ConstFact)[i], REALNAME(ConstFact)[i].exp);
#else // CACHE_COEF
	...
	s_exp = (EXP*)malloc(FACT_COEFF*sizeof(EXP));
	
	// import Bernoulli numers
	for (k = 0; k < FACT_COEFF; k++)
	{
		temp_exp = REALNAME(ImportBigIntCExt)(&temp, &bern_num[k]);
		temp2_exp = REALNAME(ImportBigIntCExt)(&temp2, &bern_den[k]);
		s_exp[k] = REALNAME(DivExt)(&s[k], &temp, temp_exp, &temp2, temp2_exp);

		kk = (2*k+1)*(2*k+2);
		if ((k & 1) != 0) kk = -kk;
		temp_exp = REALNAME(SetSIntExt)(&temp, kk);
		s_exp[k] = REALNAME(DivExt)(&s[k], &s[k], s_exp[k], &temp, temp_exp);
	}

	// calculate factorial coefficients
	mm = (REAL*)malloc(FACT_COEFF*FACT_COEFF*sizeof(REAL));
	r = (REAL*)malloc(FACT_COEFF*sizeof(REAL));
	mm_exp = (EXP*)malloc(FACT_COEFF*FACT_COEFF*sizeof(EXP));
	r_exp = (EXP*)malloc(FACT_COEFF*sizeof(EXP));

	for (n = 0; n < FACT_COEFF; n++)
	{
		REALNAME(Set0Ext)(&mm[n+0*FACT_COEFF]);
		mm_exp[n+0*FACT_COEFF] = EXPEXT_0;
	}
	
	for (n = 0; n < FACT_COEFF-1; n++)
	{
		REALNAME(Copy)(&mm[n+1*FACT_COEFF], &s[n+1]);
		mm_exp[n+1*FACT_COEFF] = s_exp[n+1];
		mm_exp[n+1*FACT_COEFF] = REALNAME(DivExt)(&mm[n+1*FACT_COEFF],
				&mm[n+1*FACT_COEFF], mm_exp[n+1*FACT_COEFF], &s[n], s_exp[n]);
	}

	REALNAME(Copy)(&r[0], &s[0]);
	r_exp[0] = s_exp[0];
	REALNAME(Copy)(&r[1], &mm[0+1*FACT_COEFF]);
	r_exp[1] = mm_exp[0+1*FACT_COEFF];
	...
	for (k = 2; k < FACT_COEFF; k++)
	{
		...
		for (n = 0; n < FACT_COEFF - k; n++)
		{
			...
			a_exp = mm_exp[n+1 + (k-2)*FACT_COEFF];
			REALNAME(Copy)(&b, &mm[n+1 + (k-1)*FACT_COEFF]);
			b_exp = mm_exp[n+1 + (k-1)*FACT_COEFF];
			REALNAME(Copy)(&c, &mm[n + (k-1)*FACT_COEFF]);
			c_exp = mm_exp[n + (k-1)*FACT_COEFF];
			REALNAME(Copy)(&mm[n + k*FACT_COEFF], &a);
			mm_exp[n + k*FACT_COEFF] = a_exp;
			if ((k & 1) == 0)
			{
				mm_exp[n + k*FACT_COEFF] = REALNAME(AddExt)(&mm[n + k*FACT_COEFF], &mm[n + k*FACT_COEFF], mm_exp[n + k*FACT_COEFF], &b, b_exp);
				mm_exp[n + k*FACT_COEFF] = REALNAME(SubExt)(&mm[n + k*FACT_COEFF], &mm[n + k*FACT_COEFF], mm_exp[n + k*FACT_COEFF], &c, c_exp);
			}
			else
			{
				mm_exp[n + k*FACT_COEFF] = REALNAME(MulExt)(&mm[n + k*FACT_COEFF], &mm[n + k*FACT_COEFF], mm_exp[n + k*FACT_COEFF], &b, b_exp);
				mm_exp[n + k*FACT_COEFF] = REALNAME(DivExt)(&mm[n + k*FACT_COEFF], &mm[n + k*FACT_COEFF], mm_exp[n + k*FACT_COEFF], &c, c_exp);
			}
		}
		REALNAME(Copy)(&r[k], &mm[0 + k*FACT_COEFF]);
		r_exp[k] = mm_exp[0 + k*FACT_COEFF];
	}
	free(s);
	free(s_exp);
	free(mm);
	...
	// save coefficients
	for (n = 0; n < FACT_COEFF; n++)
		REALNAME(Pack)(&TempConstFact[n], &r[n], r_exp[n]);
	free(r);
	free(r_exp);
#endif // CACHE_COEF
...
int ChebGen(int fnc, REAL* poly)
{
	...
	else if (GenBits == 6144)
		GEN_CHEB(real6144, Real6144)
	else if (GenBits == 8192)
		GEN_CHEB(real8192, Real8192)
	else if (GenBits == 12288)
		GEN_CHEB(real12288, Real12288)
	else
	{
		num = 0;
...
int ChebGenExt(int fnc, REAL* poly)
{
	...
	if (GenBits == 16)
		...
		GEN_CHEBEXT(real6144, real6144ext, Real6144)
	else if (GenBits == 8192)
		GEN_CHEBEXT(real8192, real8192ext, Real8192)
	else if (GenBits == 12288)
		GEN_CHEBEXT(real12288, real12288ext, Real12288)
	else
	{
		num = 0;
		printf("GenConst: Incorrect GenBits!\n");
...
void* GenImport(const REAL* temp)
{
	...
	case 6144:
		Real6144Import(&tmp6144, temp, REAL_BYTES, EXP_BITS);
		return &tmp6144;

	case 8192:
		Real8192Import(&tmp8192, temp, REAL_BYTES, EXP_BITS);
		return &tmp8192;

	case 12288:
		Real12288Import(&tmp12288, temp, REAL_BYTES, EXP_BITS);
		return &tmp12288;
	}
...
void* GenImportExt(const REAL* temp)
{
	...
	case 6144:
		tmpexp = Real6144ImportExt(&tmp6144, temp, REAL_BYTES, EXP_BITS);
		return &tmp6144;

	case 8192:
		tmpexp = Real8192ImportExt(&tmp8192, temp, REAL_BYTES, EXP_BITS);
		return &tmp8192;

	case 12288:
		tmpexp = Real12288ImportExt(&tmp12288, temp, REAL_BYTES, EXP_BITS);
		return &tmp12288;
	}
...
void GenCordAtan()
{
	...
	// number of entries
	num = CORD_ATAN_FNC(GenBaseBits*GenBaseNum); // short number of entries
	if (num > CORD_ATAN) num = CORD_ATAN;

	// generate coefficients ki = atan(1/pow(2, i))
	REALNAME(Set1)(&val); // tangent value
	for (i = 0; i < num; i++)
	{
#ifdef ANTILOCK
		REALNAME(Set1)(&TempConstAtan[i]);
#else
		REALNAME(ATan_TaylorFrom)(&TempConstAtan[i], &val);
#endif
		REALNAME(Div2)(&val);
	}

	// generate init value (=1/expansion factor) = 1/sqrt(mul(1 + 1/pow(2,2*i)))
	num = GenBaseBits*GenBaseNum; // real number of entries
	REALNAME(Set1)(&val); // first value
	REALNAME(Set1)(&TempConstAtanInit); // init accumulator
	for (i = 0; i < num; i++)
	{
		REALNAME(IncFrom)(&tmp, &val); // 1 + 1/pow(2,2*i)
		REALNAME(Mul)(&TempConstAtanInit, &TempConstAtanInit, &tmp); // mul(..)
		REALNAME(Div2)(&val); // shift 1/pow(2,2*i)
		REALNAME(Div2)(&val);
	}
	REALNAME(Sqrt)(&TempConstAtanInit); // sqrt(mul...
	REALNAME(Rec)(&TempConstAtanInit); // 1/sqrt(mul...
}

// print denormalised mantise of Cordic table (name = NULL to not print header)
void PrintCord(const char* name, const REAL* temp, const char* comment)
{
	FILE* f;
	double n;
	char bb[100];
...
void Gen(int basebits, int basenum, int expbits)
{
	...
	fprintf(f, "//\n");
	fprintf(f, "// ****************************************************************************\n");
	fprintf(f, "// PicoLibSDK, Copyright (c) 2023 Miroslav Nemecek, Panda38@seznam.cz\n\n");
	...
	PrintConst("ConstMPi05",	&TempConstMPi05,		"-pi/2");
	PrintConst("ConstPi",		&TempConstPi,			"pi");
	PrintConst("ConstMPi",		&TempConstMPi,			"-pi");
	PrintConst("ConstPi2",		&TempConstPi2,			"pi*2");
	PrintConst("ConstMPi2",		&TempConstMPi2,			"-pi*2");
	...
	PrintConstExt("ConstMPi05Ext",	&TempConstMPi05,		"-pi/2");
	PrintConstExt("ConstPiExt",		&TempConstPi,			"pi");
	PrintConstExt("ConstMPiExt",	&TempConstMPi,			"-pi");
	PrintConstExt("ConstPi2Ext",	&TempConstPi2,			"pi*2");
	PrintConstExt("ConstMPi2Ext",	&TempConstMPi2,			"-pi*2");
	...
	fprintf(f, "const REALEXT REALNAME(ConstFact)[FACT_COEFF] = {\t// FACT_COEFF = %d\n", factcoeff);
	for (i = 0; i < n; i++)
	{
		g = &TempConstFact[i];
		sprintf(bb, "a%d", i);
		PrintConstExt(NULL, g, bb);
	}
	fprintf(f, "};\n");
	fprintf(f, "#endif\n");
	...
	// generate Cordic atan table
	// Note: Second half of table is equal to angle.
	GenCordAtan();
	n = CORD_ATAN_FNC(GenBaseBits*GenBaseNum); // number of entries
	if (n > CORD_ATAN) n = CORD_ATAN;
	fprintf(f, "\n// Cordic atan coefficients\n");
	fprintf(f, "#ifdef CORD_ATAN\n");
	PrintCord("ConstAtanInit", &TempConstAtanInit, "init value of atan table (=1/expansion factor)");
	fprintf(f, "const REAL REALNAME(ConstAtan)[CORD_ATAN] = {\t// CORD_ATAN = %d\n", n);
	for (i = 0; i < n; i++)
	{
		sprintf(bb, "%d", i);
		PrintCord(NULL, &TempConstAtan[i], bb);
	}
	fprintf(f, "};\n");
	fprintf(f, "#endif\n");
...
//void Gen(int basebits, int basenum, int expbits)
	...
	Gen(32, 192, 30); // real6144
	Gen(32, 256, 30); // real8192
	Gen(32, 384, 30); // real12288
}
