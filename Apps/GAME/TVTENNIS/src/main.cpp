...
#include "../snd/bound.cpp"	// const u8 BoundSnd[]
#include "../snd/out.cpp"	// const u8 OutSnd[]
#include "../snd/paddle.cpp"	// const u8 PaddleSnd[]
#include "../snd/serve.cpp"	// const u8 ServeSnd[]

u8 *IntroImg;

#define SEGM	8	// base segment size in pixels
...
#define NUMW	((DIGW+2)*DIGSEG) // width of digit in pixels (= 48)
...
#define PADDLESPEED (0.00025f*2) // paddle speed
#define BALLSPEED (0.0003f*2) // ball speed
...
void GameControl()
{
	...
	// left player up
	if ((KeyPressed(KEY_UP) && !Comp[0]) ||
		((Tied || Comp[1]) && KeyPressed(KEY_PAD_X) && !Comp[0]) || (key0 == KEY_UP))
	{
	...
	// left player down
	if ((KeyPressed(KEY_DOWN) && !Comp[0]) ||
		((Tied || Comp[1]) && KeyPressed(KEY_PAD_A) && !Comp[0]) || (key0 == KEY_DOWN))
	{
		PaddleY[0] += TimeDelta*PADDLESPEED;
	...
	// right player up
	if ((KeyPressed(KEY_PAD_X) && !Comp[1]) ||
		((Tied || Comp[0]) && KeyPressed(KEY_UP) && !Comp[1]) || (key1 == KEY_UP))
	{
	...
	// right player down
	if ((KeyPressed(KEY_PAD_A) && !Comp[1]) ||
		((Tied || Comp[0]) && KeyPressed(KEY_DOWN) && !Comp[1]) || (key1 == KEY_DOWN))
	{
		PaddleY[1] += TimeDelta*PADDLESPEED;
...
void OpenDraw()
{
	...
	// draw title
	DrawClear();
	DrawText4("TV Tennis", (WIDTH - 9*16*2)/2, 5*2, COL_YELLOW);

#define MENUX (10*2)
#define MENUY (45*2)
#define MENUDY (16*2)
	// draw menu
	DrawText2("Press key to start the game:", MENUX, MENUY + 0*MENUDY, COL_GREEN);
	DrawText2("LEFT ...... Play with left paddle", MENUX, MENUY+1*MENUDY, COL_WHITE);
	DrawText2("RIGHT ..... Play with right paddle", MENUX, MENUY+2*MENUDY, COL_WHITE);
	DrawText2("DOWN ...... 2 players", MENUX, MENUY+3*MENUDY, COL_WHITE);
	DrawText2("UP ........ Demo", MENUX, MENUY+4*MENUDY, COL_WHITE);
	DrawText2("A ......... Play with tied paddles", MENUX, MENUY+5*MENUDY, COL_WHITE);

#define MENUY2 (MENUY+7*MENUDY)
	// draw game control
	DrawText2("Game control:", MENUX, MENUY2+0*MENUDY, COL_GREEN);
	DrawText2("LEFT or RIGHT ... Throw the ball", MENUX, MENUY2+1*MENUDY, COL_WHITE);
	DrawText2("UP/DOWN ......... Left paddle", MENUX, MENUY2+2*MENUDY, COL_WHITE);
	DrawText2("X/A ............. Right paddle", MENUX, MENUY2+3*MENUDY, COL_WHITE);
	DrawText2("Y ............... Quit the game", MENUX, MENUY2+4*MENUDY, COL_WHITE);
...
void Game(Bool comp1, Bool comp2, Bool tied)
{
	...
	while (KeyPressed(KEY_LEFT) || KeyPressed(KEY_RIGHT) ||
		...
		KeyPressed(KEY_PAD_A) || KeyPressed(KEY_PAD_X)) {}
	KeyFlush();
