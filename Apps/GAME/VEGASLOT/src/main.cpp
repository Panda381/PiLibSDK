...
#include "../img/back.cpp"	// const u8 BackImgPNG[] ... PNG
#include "../img/blur.cpp"	// const u8 BlurImgPNG[] ... PNG (128x576, 1x6 symbols of size 128x96)
#include "../img/digits.cpp"	// const u8 DigitsImg[] ... 384x48 (12x1 digits of size 32x48)
#include "../img/intro.cpp"	// const u8 IntroImgJPG[] ... JPG
#include "../img/symbols.cpp"	// const u8 SymbolsImgPNG[] ... PNG (128x576, 1x6 symbols of size 128x96)

#include "../snd/autooff.cpp"	// const u8 AutoOffSnd[]
#include "../snd/autoon.cpp"	// const u8 AutoOnSnd[]
#include "../snd/cash.cpp"	// const u8 CashSnd[]
#include "../snd/jingle.cpp"	// const u8 JingleSnd[] ... MP3
#include "../snd/out.cpp"	// const u8 OutSnd[]
#include "../snd/reel.cpp"	// const u8 ReelSnd[]
#include "../snd/risklost.cpp"	// const u8 RiskLostSnd[]
#include "../snd/riskwin.cpp"	// const u8 RiskWinSnd[]
#include "../snd/start.cpp"	// const u8 StartSnd[]
#include "../snd/stop.cpp"	// const u8 StopSnd[]
#include "../snd/toss.cpp"	// const u8 TossSnd[]
#include "../snd/transfer.cpp"	// const u8 TransferSnd[]
#include "../snd/win.cpp"	// const u8 WinSnd[]

// MP3 player
sMP3Player MP3Player;
u8 ALIGNED MP3PlayerOutBuf[MP3PLAYER_OUTSIZE];

u8 *BackImg, *BlurImg, *IntroImg, *SymbolsImg;
...
void DispVal(int val, int x, int dig)
{
	...
	for (; dig > 0; dig--)
	{
		...
		if ((val == 0) && !first)
		{
			DrawImg(DigitsImg, x, DIGITY, 0, 0, DIGITW, DIGITH);
		}
		else
		{
			k = val/10;
			val -= k*10;
			DrawImg(DigitsImg, x, DIGITY, (val+1)*DIGITW, 0, DIGITW, DIGITH);
			val = k;
			first = False;
		}
	}
...
void DispBet()
{
	...
	if (k == BET_HIGH)
		DrawImg(DigitsImg, BETX, DIGITY, 11*DIGITW, 0, DIGITW, DIGITH);
	else
...
void DispReel(u8 inx)
{
	...
	while (h > 0)
	{
		...
		if (r->speed >= REEL_BLURSPEED)
			DrawImg(BlurImg, x, y, 0, tab[pos]*SYMBOLH + ys, SYMBOLW, h2);
		else
			DrawImg(SymbolsImg, x, y, 0, tab[pos]*SYMBOLH + ys, SYMBOLW, h2);
...
#define SHADOWDH 1
	...
	for (i = 1; i <= 63; i++)
	{
		DrawRect(x, y, SYMBOLW, SHADOWDH, COLORA(0, 0, 0, ((64-i)*4)));
		y += SHADOWDH;
	}
	...
	// win lines darkness
	if (WinLines < 2) DrawRect(x, REELY, SYMBOLW, SYMBOLH, COLORA(0, 0, 0, 5*15));
	if (WinLines < 1) DrawRect(x, REELY+SYMBOLH, SYMBOLW, SYMBOLH, COLORA(0, 0, 0, 5*15));
	if (WinLines < 3) DrawRect(x, REELY+2*SYMBOLH, SYMBOLW, SYMBOLH, COLORA(0, 0, 0, 5*15));
}
...
void TurnReel(Bool fast)
{
	...
	if (fast)
	{
		PlaySound(ReelSnd, 1, SNDREPEAT_REPEAT, 1, 0.3f);
		PlaySound(StartSnd);
	}
		...
		for (inx = 0; inx < REEL_NUM; inx++)
		{
			...
			else
			{
				...
				if ((r->rem > 0) || (r->over > 0))
				{
					...
					if ((r->rem <= 0) && (r->over <= 0) && fast) PlaySound(StopSnd, 0, SNDREPEAT_NO, 1, 0.6f);
				}
			}
...
void DoToss()
{
	...
	PlaySound(TossSnd, 1, SNDREPEAT_REPEAT, 1, 0.5f);
		...
		for (;;)
		{
			...
			if ((u32)(Time() - t) >= 1000000)
			{
				...
				if (ch == KEY_PAD_A)
				{
					...
					{
						// odd: lost
						PlaySound(RiskLostSnd, 0, SNDREPEAT_NO, 1, 0.3f);
						n = 0;
					}
					else
					{
						// even: win
						PlaySound(RiskWinSnd, 0, SNDREPEAT_NO, 1, 1);
						n *= 2;
						AddBank(n);
					}
...
int main()
{
	...
	// decompress images
	BackImg = (u8*)PNGLOAD(BackImgPNG); if (BackImg == NULL) Reboot();
	BlurImg = (u8*)PNGLOAD(BlurImgPNG); if (BlurImg == NULL) Reboot();
	IntroImg = (u8*)JPGLOAD(IntroImgJPG); if (IntroImg == NULL) Reboot();
	SymbolsImg = (u8*)PNGLOAD(SymbolsImgPNG); if (SymbolsImg == NULL) Reboot();
	...
	int r = MP3PlayerInit(mp3, NULL, JingleSnd, sizeof(JingleSnd), MP3PlayerOutBuf, MP3PLAYER_OUTSIZE, -1);
	if (r == ERR_MP3_OK) MP3Play(mp3, 0, False);

	// wait a key (max. 10 seconds)
	u32 t1 = Time();
	WaitMs(200);
	KeyFlush();
	while ((u32)(Time() - t1) < 10000000)
	{
		ch = KeyGet();
		if (ch == KEY_PAD_Y)
			Reboot();
