...
void DispAll(u32 wait)
{
	...
	for (y = 0; y < MAPH; y++)
	{
		for (x = 0; x < MAPW; x++)
		{
			DrawImg(TilesImg, x*TILEW+MAPX, y*TILEH+MAPY, (*s++)*TILEW, 0, TILEW, TILEH);
		}
	}

	// fruit
	if (FruitSpriteImg >= 0)
		DrawImg(SpritesTransImg, FruitSpriteX,
			FruitSpriteY, FruitSpriteImg*SPRITEW, 0, SPRITEW, SPRITEH);
	...
	for (i = 0; i < 5; i++)
	{
		if (i < Lives)
			DrawImg(StatusImg, i*STATUSW+MAPX,
				HEIGHT-STATUSH, 0, 0, STATUSW, STATUSH);
		else
	...
	for (i = 0; i < 7; i++)
	{
		...
		if (j <= k)
			DrawImg(StatusImg, MAPX+MAPWIDTH-(i+1)*STATUSW,
				HEIGHT-STATUSH, (FruitTab[j]+1)*STATUSW, 0, STATUSW, STATUSH);
		else
	...
	for (i = 0; i < 5; i++)
	{
		...
		// display sprite
		DrawImg(SpritesTransImg, ch->x, ch->y, k*SPRITEW, 0, SPRITEW, SPRITEH);
	}
		...
		if (ch->steps <= 0)
		{
			...
			if (i == CH_PACMAN)
			{
				...
				if ((FrightenCnt > 0) && !PlayingSound())
					SETNEXTSOUND(FrightenSnd);

				// collect dot
				tile = GetTile(xpac, ypac);
				if (tile == TILE_DOT)
				{
					if (FrightenCnt == 0) SETNEXTSOUND(WakkaSnd);
				...
				{
					if (FrightenCnt == 0) SETNEXTSOUND(SirenSnd);
				}
...
void NewGame(Bool cont)
{
	...
	// start default sound - needed to set correct stream format of the sound
	PlaySound(SirenSnd);
}
