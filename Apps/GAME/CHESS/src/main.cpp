...
#include "../img/pieces.cpp"	// const u8 PiecesImgPNG[] 392x448 (7x8 tiles, 1 tile 56x56)

u8 *IntroImg, *PiecesImg;

//#define DEBUG_STACK	// debug flag - display stack max. depth

// text console
#define FONTW2	16	// font width
#define FONTH2	32	// font height
#define TEXTX	480	// text X coordinate
#define TEXTW	10	// text width (10 characters, 160 pixels)
#define TEXTH	15	// text height (15 rows, 480 pixels)
#define TEXTS (TEXTW*TEXTH) // size of text buffer

// field tile
#define TILEW	56	// tile width
#define TILEH	56	// tile height
...
// visible board
#define FRAME	16	// frame width
#define BOARDX	FRAME	// board X coordinate
#define BOARDY	FRAME	// board Y coordinate
#define BOARDW (2*FRAME + MAP0W*TILEW) // board total width (= 480)
#define BOARDH (2*FRAME + MAP0H*TILEH) // board total height (= 480)
...
#define STACK_MAX	10000	// max. size of move stack
#define DEEP_MAX	6	// max. deep in move stack
...
void OutText(string txt)
{
	...
	memcpy(&TextBuf[TextRows*TEXTW], txt.str(), len);
	...
	for (y = 0; y < TEXTH; y++)
	{
		...
		TextBuf[(y+1)*TEXTW] = 0;
		DrawTextBg2(&TextBuf[y*TEXTW], WIDTH - TEXTW*FONTW2, y*FONTH2, COL_WHITE, COL_BLACK);
...
void DispField(u8 inx, u8 w, u32 color, Bool usemove, Bool usecatch)
{
	...
	// display field
	DrawImg(PiecesImg, x, y, xs, ys, TILEW, TILEH);
...
void DispFrame()
{
	...
	for (i = 0; i < MAP0W; i++)
	{
		...
		DrawText2(buf, FRAME + TILEW/2 - 8 + i*TILEW, 0, LEGENDCOL); // top legend
		DrawText2(buf, FRAME + TILEW/2 - 8 + i*TILEW, HEIGHT-FRAME, LEGENDCOL); // top legend
	}
	...
	for (i = 0; i < MAP0H; i++)
	{
		...
		DrawText2(buf, 0, FRAME + TILEH/2 - 8 + i*TILEH, LEGENDCOL); // left legend
		DrawText2(buf, BOARDW-FRAME, FRAME + TILEH/2 - 8 + i*TILEH, LEGENDCOL); // left legend
	}
}
...
#define MENUDY 16
#define MENUY 390
#define MENUH (5*MENUDY+12)

		// draw rectangle
		DrawRect((WIDTH-MENUW)/2, MENUY-6, MENUW, MENUH, COLORA(0, 0, 0, 160));

		// draw menu
		DrawText("LEFT .... Play with white",	MENUX, MENUY+0*MENUDY, COL_WHITE);
...
#define MENUY2 (MENUY+4*MENUDY)
		...
		// draw level
		if (DeepMax == 2)
			DrawText("BEGINNER",	MENUX2, MENUY2, COL_WHITE);
		else if (DeepMax == 3)
			DrawText("ADVANCED",	MENUX2, MENUY2, COL_WHITE);
		else if (DeepMax == 4)
			DrawText("PRO",		MENUX2, MENUY2, COL_WHITE);
		else if (DeepMax == 5)
			DrawText("SUPERPRO",	MENUX2, MENUY2, COL_WHITE);
		else //if (DeepMax == 6)
			DrawText("HERO",	MENUX2, MENUY2, COL_WHITE); // 4 minutes
//		else
//			DrawText("IMPOSSIBLE",	MENUX2, MENUY2, COL_WHITE);
...
int main()
{
	...
	PiecesImg = (u8*)PNGLOAD(PiecesImgPNG); if (PiecesImg == NULL) Reboot();
	...
	while (True)
	{
		...
		while (True)
		{
			...
			if (MoveNum < 10) txt += ' ';
			if (MoveNum < 100) txt += ' ';
			txt.AddUInt(MoveNum, 0);
			txt += ':';
			if (Player == BLACK_PLAYER) txt = "    ";
			...
			else
			{
				...
				// take piece
				if (LastMove->dstpiece != EMPTY) txt += 'x';

				// En Passant
				if (LastMove->flags == MOVEFLAG_EP) txt += "e";
			}	
