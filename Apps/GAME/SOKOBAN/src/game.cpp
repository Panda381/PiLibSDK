...
// player save screen
u32 SaveScr[32*32];	// save screen buffer
...
void DispTile(u8 x, u8 y)
{
	...
	DrawImg(TilesImg, x*TileSize + LevelX, y*TileSize + LevelY,
...
void RestorePlayer()
{
	...
	{
		u32* s = SaveScr;
		u32* d = &FrameBuffer.drawbuf[SaveScrX + SaveScrY*WIDTH];
		int h;
		for (h = TileSize; h > 0; h--)
		{
			memcpy(d, s, TileSize*4);
...
void SetPlayer(int x, int y, u8 tile)
{
	...
	RestorePlayer();
	SavePlayer(x, y);
	DrawImg(TilesImg, x, y, tile*TileSize, 0, TileSize, TileSize);
}
...
void LevelInfo()
{
	...
	// print author's name
	DRAWTEXT(SelAuthor, 0, FONTH*DISPZOOM, COL_WHITE);
	DRAWTEXT(AutName, 8*8*DISPZOOM, FONTH*DISPZOOM, COL_GREEN);

	// print collection name
	DRAWTEXT(SelColl, 0, 2*FONTH*DISPZOOM, COL_WHITE);
	DRAWTEXT(CollName, 12*8*DISPZOOM, 2*FONTH*DISPZOOM, COL_GREEN);

	// print level number
	DRAWTEXT("LEVEL:", 0, 3*FONTH*DISPZOOM, COL_WHITE);
	char buf[12];
	DecNum(buf, Level+1, 0);
	DRAWTEXT(buf, 7*8*DISPZOOM, 3*FONTH*DISPZOOM, COL_GREEN);
...
void GameLoop()
{
	...
	while (True)
	{
		...
		if (i != NOKEY)
		{
			...
			// next scene
			else if ((i == KEY_END)) //RIGHT) && KeyPressed(KEY_ALT))
			{
				Level++;
			...
			// previous scene
			else if ((i == KEY_HOME)) //LEFT) && KeyPressed(KEY_ALT))
			{
				Level--;
			...
			// select fast mode
			else if ((i == KEY_PGUP)) //UP) && KeyPressed(KEY_ALT))
			{
				FastMode = True;
			}

			// select slow mode
			else if ((i == KEY_PGDN)) //DOWN) && KeyPressed(KEY_ALT))
			{
				FastMode = False;
			}
