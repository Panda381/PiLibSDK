...
void DispShip()
{
	...
	if (ShipInCrash > 0)
	{
		if (ShipInCrash < SHIP_CRASH_NUM)
		{
			...
			DrawImg(ExplosionImg, (ShipX>>FRAC)-EXPLOSION_IMG_W/2, y-15,
				0, ShipInCrash*EXPLOSION_IMG_H,
				EXPLOSION_IMG_W, EXPLOSION_IMG_H);
		}
		return;
	}
	...
	if (SubLevel != 3)
	{
		DrawImg(ShipImg, (ShipX>>FRAC)-SHIP_IMG_W/2, HEIGHT-20,
			0, SHIP_SHADOW*SHIP_IMG_H, SHIP_IMG_W, SHIP_IMG_H);
	}
	...
	y = HEIGHT-30-(Speed>>(FRAC+3));
	if (SubLevel == 3) y = HEIGHT-40;
	DrawImg(ShipImg, (ShipX>>FRAC)-SHIP_IMG_W/2, y,
		0, phase*SHIP_IMG_H, SHIP_IMG_W, SHIP_IMG_H);
}
