...
NEWGAME:
	...
	{
		Tiny_Flip(1,&space);
		if (KeyPressed(KEY_PAD_A)) //digitalRead(1)==0)
		{
			Sound(100,125);
			Sound(50,125);
			goto BYPASS2;
...
//		_delay_ms(10);//Delay because is too fast with FastTinyDriver!
		...
		if (ShipDead!=1)
		{
			...
			if (KeyPressed(KEY_PAD_A) /*(digitalRead(1)==0)*/ &&(MyShootReady==SHOOTS))
			{
				Sound(200,4);
				MyShootReady=0;
				space.MyShootBall=6;
...
const u32 TInvader_BgCol[8] = { COLOR(5*32,2*32,3*32), COL_MAGENTA, COLOR(7*32,0,2*32), COL_RED, COLOR(7*32,5*32,0), COL_YELLOW, COLOR(5*32,7*32,0), COLOR(0,7*32,1*32) };
...
void Tiny_Flip(uint8_t render0_picture1,SPACE *space)
{
	...
	if (KeyPressed(KEY_ALT) && KeyPressed(KEY_PAD_X))
	{
//		DispUpdate();
		ScreenShot();
	}
