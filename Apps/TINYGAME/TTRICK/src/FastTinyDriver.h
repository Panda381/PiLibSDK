...
#define BUTTON_DOWN (KeyPressed(KEY_PAD_A) || KeyPressed(KEY_PAD_B)) //(digitalRead(1) == 0)
#define BUTTON_UP (!KeyPressed(KEY_PAD_A) && !KeyPressed(KEY_PAD_B)) //(digitalRead(1) == 1)
...
// - After write all data, send image to display with DispUpdate().
INLINE void DispDirectDraw(int x, int y, u8 data, u32 col) { DispI2C_Add(x, y, data, col); }
INLINE void DispDirectDraw2(u8 data, int x, int y, u32 col) { DispI2C_Add(x, y, data, col); }
