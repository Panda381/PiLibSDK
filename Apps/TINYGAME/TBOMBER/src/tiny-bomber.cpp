...
void StartGame(void)
{
	if (INGAME==0)
	{
		while(1)
		{
			if (!KeyPressed(KEY_PAD_A)/*digitalRead(1)==1*/) break;
		}
		_delay_ms(40);
		INGAME=1;
	}
...
RESTARTLEVEL:
	...
	while(1)
	{
		if (KeyPressed(KEY_PAD_A)/*digitalRead(1)==0*/) StartGame();
		...
		if (INGAME)
		{
			...
			if (KeyPressed(KEY_PAD_A)/*(digitalRead(1)==0)*/ && (BOMBXY[2]==0))
			{
